...
namespace mkfit {
  ...
  void MkFinder::setup_bkfit(const PropagationConfig &pc, const SteeringParams &sp, const Event *ev, const IterationParams &ip) {
    m_prop_config = &pc;
    m_steering_params = &sp;
    m_iteration_params = &ip;
    m_event = ev;
  ...
  void MkFinder::findCandidates(const LayerOfHits &layer_of_hits,
    ...
    for (int itrack = 0; itrack < NN; ++itrack) {
      ...
      newcand.setScore(getScoreCand(
          m_steering_params->m_track_scorer, *m_iteration_params, newcand, true /*penalizeTailMissHits*/, true /*inFindCandidates*/));
