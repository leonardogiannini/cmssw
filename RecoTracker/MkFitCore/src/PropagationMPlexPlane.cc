...
#include "JacErrPropCurv2.ah"
  ...
  void parsAndErrPropFromPathL_impl(const MPlexLV& __restrict__ inPar,
    ...
    const MPF u11 = -sinPin; //-au * t12;
    const MPF u12 = cosPin; //au * t11;
    const MPF v11 = -cosT * u12;
    const MPF v12 = cosT * u11;
    const MPF v13 = sinT; //t11 * u12 - t12 * u11;
    //au = mpt::fast_isqrt(t21 * t21 + t22 * t22);
    const MPF u21 = -sinPout; //-au * t22;
    const MPF u22 = cosPout; //au * t21;
    const MPF v21 = -cosT * u22;
    const MPF v22 = cosT * u21;
    const MPF v23 = sinT; //t21 * u22 - t22 * u21;
...
#ifdef DEBUG
    if (debug)
      std::cout << "A=" << A << " B=" << B << " C=" << C << " s1=" << s1 << std::endl; //<< " s2=" << s2 << std::endl;
#endif
    //take the closest
    return s1; //(std::abs(s1) > std::abs(s2) ? s2 : s1);
  }
