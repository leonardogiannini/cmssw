...
void MkBuilder::fit_tracks(MkFinder *mkfndr, int nFoundHits, std::vector<int> inds, int start_trk, int end_trk) {
    ...
    const PropagationConfig &prop_config = m_job->m_trk_info.prop_config();
    mkfndr->m_prop_config = &prop_config;
...
void MkBuilder::check_tracks(std::vector<int> inds, int start_trk, int end_trk) {
    ...
    for (int icand = start_trk; icand < end_trk; icand += NN) {
     const int end = std::min(icand + NN, end_trk);
      ...
      std::cout << " this is the region " << " strt "<<start_trk << " end " << end_trk << " end "<<  end_trk-start_trk<<" NN " << NN << std::endl;
  ...
  void MkBuilder::fittracks() {
	...
	if(mapFoundHits.count(foundh)) { mapFoundHits[foundh].push_back(N);  }
	else mapFoundHits[foundh]={N};
	N++;
    }
#ifdef DEBUG_FIT
...
#ifdef DEBUG_FIT
	std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" <<std::endl;
	std::cout << "ntimes " << ntimes << " size  "<< m.second.size()  << " extra "<< m.second.size()-NN*ntimes <<std::endl;
#endif
