...
namespace mkfit {
  ...
  void MkBuilder::fit_tracks(MkFinder *mkfndr, int nFoundHits, std::vector<int> inds, int start_trk, int end_trk) {
    //should be wrapped into some setup_fit
    ...
    mkfndr->my_flags = &my_flags;

    //     const SteeringParams &st_par = m_job->steering_params(region);
    //     const PropagationConfig &prop_config = m_job->m_trk_info.prop_config();

    //     mkfndr->m_steering_params = &st_par;
    mkfndr->m_event = m_event;

    //     std::cout << " pflag check "  << std::endl;
    //     std::cout <<mkfndr->m_prop_config->finding_inter_layer_pflags.use_param_b_field<< std::endl;
    //     std::cout <<mkfndr->m_prop_config->finding_inter_layer_pflags.apply_material<< std::endl;

    //     mkfndr->setup_ffit(prop_config);
    ...
    for (int icand = start_trk; icand < end_trk; icand += NN) {
      ...
      //       fwdFitOutput
      mkfndr->fwdFitOutputTracks(m_tracks, inds, icand, end, nFoundHits);

      //       //not sure if I need to reconfigure prop_config for bkw or not
      //       // input candidate tracks
      ...
      // fwdFitOutput
      mkfndr->fwdFitOutputTracks(m_tracks, inds, icand, end, nFoundHits, true);

      //just a print should not do it
      //mkfndr->fwdFitInputTracks(m_tracks, inds, icand, end);
    }
    //     std::cout << mkfndr->m_prop_config->finding_requires_propagation_to_hit_pos << std::endl;
  ...
  void MkBuilder::fittracks() {
    ...
    for (auto &t : m_tracks) {
      ...
      // 	}
      //     if (t.getHitIdx(0)<0 && t.getHitLyr(0)==1) {N++;continue;}
