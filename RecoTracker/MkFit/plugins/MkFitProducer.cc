...
void MkFitProducer::produce(edm::StreamID iID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  auto cpe = [&](int orig_hit_idx, float ltp_arr[6], float(&hit_arr)[5]) -> bool {
    ...
    //   //run-cpe with cluster etc;
    //   //return false if something is wrong
    //std::cout << "GLOBAL x " << hit.globalPosition().x() << " y " << hit.globalPosition().y() << " z " << hit.globalPosition().z() << std::endl;
    //std::cout << "x " << hit.localPosition().x() << " y " << hit.localPosition().y() << std::endl;
    //std::cout << "xx " << hit.localPositionError().xx() << " xy " << hit.localPositionError().xy() << " yy " << hit.localPositionError().yy() << std::endl;
    //std::cout << "run cpe" << std::endl;
