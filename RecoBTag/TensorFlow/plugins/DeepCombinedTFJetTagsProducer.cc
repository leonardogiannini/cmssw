...
struct DeepCombinedTFCache {
  DeepCombinedTFCache() : graphDef(nullptr) {
  }

  std::atomic<tensorflow::GraphDef*> graphDef;
}; 

class DeepCombinedTFJetTagsProducer : public edm::stream::EDProducer<edm::GlobalCache<DeepCombinedTFCache>> {

  public:
    explicit DeepCombinedTFJetTagsProducer(const edm::ParameterSet&, const DeepCombinedTFCache*);
    ~DeepCombinedTFJetTagsProducer() override;

    static void fillDescriptions(edm::ConfigurationDescriptions&);

    static std::unique_ptr<DeepCombinedTFCache> initializeGlobalCache(const edm::ParameterSet&);
    static void globalEndJob(const DeepCombinedTFCache*);
...
DeepCombinedTFJetTagsProducer::DeepCombinedTFJetTagsProducer(const edm::ParameterSet& iConfig,
  const DeepCombinedTFCache* cache) :
...
DeepCombinedTFJetTagsProducer::~DeepCombinedTFJetTagsProducer()
{
...
void DeepCombinedTFJetTagsProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
    
  // pfDeepCombinedJetTags
  ...
  desc.add<std::vector<std::string>>("input_names", 
    ...
    {   
        "input_1_DFla", "input_2_DFla", "input_3_DFla", "input_4_DFla",                                             //glob, cpf, npf, svs
        "input_1", "input_2",                                                                                       //glob2, seeding tracks
        "input_3", "input_4",  "input_5","input_6","input_7","input_8","input_9","input_10","input_11","input_12"   // clusters of neighbors
        
    });
  
  desc.add<edm::FileInPath>("graph_path",
    edm::FileInPath("RecoBTag/Combined/data/DeepVertex/phase1_deepvertexcombined.pb"));
...
std::unique_ptr<DeepCombinedTFCache> DeepCombinedTFJetTagsProducer::initializeGlobalCache(
  const edm::ParameterSet& iConfig)
{
  ...
  DeepCombinedTFCache* cache = new DeepCombinedTFCache();
  cache->graphDef = tensorflow::loadGraphDef(pbFile);

  return std::unique_ptr<DeepCombinedTFCache>(cache);
}

void DeepCombinedTFJetTagsProducer::globalEndJob(const DeepCombinedTFCache* cache)
{
...
void DeepCombinedTFJetTagsProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  std::vector<tensorflow::TensorShape> input_sizes {
    ...
    {n_batch_jets, 4},         // input_5 - global jet features
    {n_batch_jets, 10, 21},     // input_6 - seeding tracks
    ...
    {n_batch_jets,20, 36},      // input_13 - neighbours  
    {n_batch_jets,20, 36},      // input_14 - neighbours  
    {n_batch_jets,20, 36},      // input_15 - neighbours  
    {n_batch_jets,20, 36},      // input_16 - neighbours     
  ...
  for (std::size_t batch_n=0; batch_n < n_batches; batch_n++) {
    ...
    for (std::size_t jet_bn=0; jet_bn < (std::size_t) n_batch_jets; jet_bn++) {
      ...
      // c_pf candidates
      auto max_c_pf_n = std::min(features.c_pf_features.size(), (std::size_t) input_sizes.at(kChargedCandidates).dim_size(1));
      ...
      // n_pf candidates
      auto max_n_pf_n = std::min(features.n_pf_features.size(), (std::size_t) input_sizes.at(kNeutralCandidates).dim_size(1));
      ...
      // jet and other global features    
      input_tensors.at(kGlobal1).second.matrix<float>()(jet_bn, 0) = (features.jet_features.pt);
      input_tensors.at(kGlobal1).second.matrix<float>()(jet_bn, 1) = (features.jet_features.eta);
      input_tensors.at(kGlobal1).second.matrix<float>()(jet_bn, 2) = (features.jet_features.phi);
      input_tensors.at(kGlobal1).second.matrix<float>()(jet_bn, 3) = (features.jet_features.mass);
...
DEFINE_FWK_MODULE(DeepCombinedTFJetTagsProducer);
